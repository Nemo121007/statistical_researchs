...
def main():
    ...
    for file in files:
        ...
        dist_cosine = spherical_law_of_cosines_distance(
            calc_data['lat1'].values, calc_data['lon1'].values,
            calc_data['lat2'].values, calc_data['lon2'].values
        )
        ...
        for method in ['haversine', 'cosine']:
            ...
            temp_result[f'{method}_max_underestimation_m'] = abs(
                negative_errors.min()) if not negative_errors.empty else 0.0
