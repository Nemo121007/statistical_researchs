...
def _get_speed_intervals(
    ...
    """Вспомогательный метод для получения интервалов, разбитых по скорости."""
...
def filter_intervals_by_speed_and_length(
    ...
    for start, end in intervals:
        if end > start and float(
            np.sum(segment_distances(lat_rad[start : end + 1], lon_rad[start : end + 1]))
        ) >= min_interval_length:
...
if __name__ == "__main__":
    ...
    for i in range(15):
        ...
        check_lon, check_lat, check_time, check_validate_point = (
            filter_intervals_by_speed_and_length(
                lon_df,
                lat_df,
                time_df,
