...
def discretize_df(
    df: pd.DataFrame, max_len_part: int, max_distance: float = 500.0
) -> List[pd.DataFrame]:
    """
...
def plot_results(smoothed_state_means: np.ndarray, data: np.ndarray, name_file: Path):
    ...
    if np.any(valid_idx):
        plt.plot(
            data[valid_idx, 0], data[valid_idx, 1], "ro", label="Наблюдения", alpha=0.5
        )
...
def test_calman_filter(list_df: List[pd.DataFrame], min_len: int = 30, m: int = 10):
    ...
    pbar = tqdm(
        enumerate(list_df), total=len(list_df), desc="Анализ чанков", unit="chunk"
    )
    ...
    for i, df in pbar:
        ...
        pbar.set_postfix_str(
            f"Type={log_type}, Idx={current_idx}, Skipped={skipped_count}"
        )
