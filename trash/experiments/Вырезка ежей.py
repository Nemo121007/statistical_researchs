...
def calculate_total_distance(points_df, name):
    ...
    time_array = (
        points_df["time"].values
        if "time" in points_df.columns
        else np.arange(len(points_df))
    )
    ...
    IOPs_geojson.write_geojson_from_arrays(
        ...
        ],  # Список с одним элементом [time, lat, lon]
    )
    return total_distance


dist_haversin = np.nansum(
    CalculatorDistancesLengthLargeCircle.vectorized_segment_distances(
        lat_array=df["lat"].to_numpy(), lon_array=df["lon"].to_numpy()
    )
)
print_count_NaN(df)
...
result_df = pd.DataFrame({"lon": list_lon, "lat": list_lat})
...
print(f"\n--- Расчет расстояния ---")
print(
    f"Расстояние по формуле гаверсин: {dist_haversin:.2f} метров = {dist_haversin/1000:.2f} км = {dist_haversin / 1000 /1.85:.2f} м. миль"
)
...
print(
    f"Расстояние после фильтрации ёжика: {dist_hedgehog:.2f} метров = {dist_hedgehog/1000:.2f} км = {dist_hedgehog / 1000 /1.85:.2f} м. миль"
)
...
print(
    f"Расстояние после упрощения (Дуглас-Пекер): {simplified_distance:.2f} метров = {simplified_distance/1000:.2f} км = {simplified_distance / 1.85 / 1000:.2f} м. миль"
)
print(
    f"Расстояние после сглаживания (Савицки-Голей): {smoothed_distance:.2f} метров = {smoothed_distance/1000:.2f} км = {smoothed_distance / 1.85 / 1000:.2f} м. миль"
)
