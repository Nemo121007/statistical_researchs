...
class Area:
    ...
    def outer_border(self, new_outer_border: List[Node]) -> None:
        if not isinstance(new_outer_border, list) or not all(
            isinstance(node, Node) for node in new_outer_border
        ):
    ...
    def inner_borders(self, new_inner_borders: List[List[Node]]) -> None:
        if not isinstance(new_inner_borders, list) or not all(
            isinstance(border, list) and all(isinstance(node, Node) for node in border)
            for border in new_inner_borders
        ):
    ...
    def bounding_box(self) -> Optional[List[float]]:
        ...
        """
        if (
            self._min_lon is None
            or self._max_lon is None
            or self._min_lat is None
            or self._max_lat is None
        ):
            return None
